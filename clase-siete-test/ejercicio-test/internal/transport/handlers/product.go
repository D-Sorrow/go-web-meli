...
import (
	...
	"github.com/D-Sorrow/go-web-meli/clase-siete-test/ejercicio-test/internal/transport/utils/validate"
	res "github.com/D-Sorrow/go-web-meli/clase-siete-test/ejercicio-test/plataform/web/response"
...
func (product *Product) GetProductById(response http.ResponseWriter, request *http.Request) {
	...
	productId, errConv := strconv.Atoi(chi.URLParam(request, "id"))
	if errConv != nil {
		res.SetError(response, http.StatusBadRequest, "Error id is not valid")
	}

	productById, err := product.productService.GetProductById(productId)

	if err != nil {
		res.SetError(response, http.StatusNotFound, "Error get product by id")
		return
	}
...
func (p *Product) UpdateProduct(response http.ResponseWriter, request *http.Request) {
	...
	if err != nil {
		res.SetError(response, http.StatusBadRequest, "error decode json")
		return
	}
	if errValidate := validate.ValidateId(productUpdate.Id); errValidate != nil {
		res.SetError(response, http.StatusBadRequest, errValidate.Error())
		return
	}
...
func (p *Product) PatchProduct(response http.ResponseWriter, request *http.Request) {
	...
	if err != nil {
		res.SetError(response, http.StatusNotFound, "error update product")
		return
	}
...
func (p *Product) DeleteProduct(response http.ResponseWriter, request *http.Request) {
	...
	idProduct, errConv := strconv.Atoi(chi.URLParam(request, "id"))
	...
	if err != nil {
		res.SetError(response, http.StatusNotFound, "error delete product")
		return
	}
	response.WriteHeader(http.StatusNoContent)
}
